...
public class ClientsPage2 extends TestBaseParallelTesting {

	boolean count = false;

	public ClientsPage2() throws IOException {
		// super();
	...
	public boolean validatelogo() {
		WebDriverWait wait = new WebDriverWait(getdriver(), 30); // wait for 5 seconds
	...
	public int validatetotaltabs() {
		...
		int tabcount = getdriver().findElements(By.xpath("//button[starts-with(@class,'mat-button ')]")).size();
	...
	public String validatesRemotelogin() throws InterruptedException {
		...
		System.out.println(" Parent tittle is " + getdriver().getTitle());

		String newWindow = it.next();
		System.out.println(" Child win id is " + newWindow);

		getdriver().switchTo().window(newWindow);
		System.out.println(" Child tittle is " + getdriver().getTitle());
	...
	public boolean items25() throws InterruptedException {
		int rows_count;
		List<WebElement> rows_table = getdriver()
				.findElements(By.xpath("//mat-row[@class='mat-row ng-star-inserted']"));
		...
		JavaScriptUtil.scrollIntoView(rangelabel, getdriver());
		JavaScriptUtil.drawBorder(rangelabel, getdriver());
	...
	public ClientDetailsPage clientclick() throws IOException, InterruptedException {
		...
		WebElement cn = getdriver().findElement(By.xpath("//a[@title='" + clientname + "']"));
